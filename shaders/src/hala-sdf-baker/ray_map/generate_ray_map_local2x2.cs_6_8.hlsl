...
void main(uint3 id: SV_DispatchThreadID) {
  ...
  for (uint i = start_triangle_id; i < end_triangle_id && (i < _upper_bound_count - 1); i++) {
    ...
    test_intersection_6_rays(tri, int3(id.xyz), intersect_forward, intersect_backward);

    // Save the forward intersection count(-axis is negative, +axis is positive) to the current cell.
    _ray_map_rw[id.xyz] += float4(intersect_forward, 1.0f);

    // Save the x direction backward intersection count(-axis is negative, +axis is positive) to the left cell.
    if (id.x > 0) {
      _ray_map_rw[int3(id.x - 1, id.y, id.z)] += float4(intersect_backward.x, 0.0f, 0.0f, 1.0f);
    }

    // Save the y direction backward intersection count(-axis is negative, +axis is positive) to the bottom cell.
    if (id.y > 0) {
      _ray_map_rw[int3(id.x, id.y - 1, id.z)] += float4(0.0f, intersect_backward.y, 0.0f, 1.0f);
    }

    // Save the z direction backward intersection count(-axis is negative, +axis is positive) to the front cell.
    if (id.z > 0) {
